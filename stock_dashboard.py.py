...
def calculate_metrics(data):
    # ... (last_close, prev_close, change, pct_change calculations remain the same)
    last_close = data['Close'].iloc[-1].item()
    prev_close = data['Close'].iloc[0].item()
    change = last_close - prev_close
    pct_change = (change / prev_close) * 100

    # 🌟 CRITICAL FIX: Use .item() to ensure max() and min() return scalars.
    high = data['High'].max().item()
    low = data['Low'].min().item()
    
    # Volume is often already a scalar, but we ensure it for safety:
    volume = data['Volume'].sum().item() 
    
    return last_close, change, pct_change, high, low, volume

# Add simple moving average (SMA) and exponential moving average (EMA) indicators
# Modified code (The Ultimate Guaranteed Fix):
def add_technical_indicators(data):
    # 1. Extract the underlying values as a NumPy array
    close_array = data['Close'].values 
    
    # 2. 🌟 CRITICAL FIX: Explicitly reshape the array to 1-dimensional (1D).
    #    The (-1) tells NumPy to calculate the size automatically (390).
    close_array = close_array.reshape(-1) 
    
    # 3. Re-create the Pandas Series from the now guaranteed 1D array
    #    The error should now be resolved here.
    close_series = pd.Series(close_array, index=data.index)
...
for symbol in stock_symbols:
    ...
    if not real_time_data.empty:
       ...
       # 🌟 CRITICAL FIX: Ensure ALL code for calculation and display is INSIDE the "if not empty" block.
       # The Indentation must be correct here.
       last_price = real_time_data['Close'].iloc[-1].item() 
       first_open = real_time_data['Open'].iloc[0].item() 
